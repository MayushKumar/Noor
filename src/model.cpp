...
namespace Noor
{
	...
	// void calculate_tan_bitan(Ref<Mesh> mesh, tinygltf::Buffer &index_buffer);
	// void calc_tan(glm::vec3 e1, glm::vec3 e2, glm::vec2 tce1, glm::vec2 tce2);
...
// 				Util::Image base_image = Util::flip_image_vertically({(uint32_t)tiny_base_image.width, (uint32_t)tiny_base_image.height, (uint32_t)tiny_base_image.component, tiny_base_image.image.data()});
// 				mesh->material->base_tex = create_texture({1, NOOR_RGB8, NOOR_LINEAR, NOOR_LINEAR, NOOR_REPEAT}, base_image);
				
// 				tinygltf::Image &tiny_normal_image = tiny_model.images[tiny_model.textures[tiny_material.normalTexture.index].source];
// 				Util::Image normal_image = Util::flip_image_vertically({(uint32_t)tiny_normal_image.width, (uint32_t)tiny_normal_image.height, (uint32_t)tiny_normal_image.component, tiny_normal_image.image.data()});
// 				mesh->material->normal_tex = create_texture({1, NOOR_RGB8, NOOR_LINEAR, NOOR_LINEAR, NOOR_REPEAT}, normal_image); 
...
// 				Util::Image metallic_roughness_image = Util::flip_image_vertically({(uint32_t)tiny_metallic_roughness_image.width, (uint32_t)tiny_metallic_roughness_image.height, (uint32_t)tiny_metallic_roughness_image.component, tiny_metallic_roughness_image.image.data()});
// 				mesh->material->metallic_roughness_tex = create_texture({1, NOOR_RGB8, NOOR_LINEAR, NOOR_LINEAR, NOOR_REPEAT},  metallic_roughness_image);
				
// 				Util::free_image(base_image);
// 				Util::free_image(normal_image);
// 				Util::free_image(metallic_roughness_image);

// 				// tinygltf::Image image = Util::load_image(, uint32_t desired_channels = 0, bool flip_vertically = true)

// 				// calculate_tan_bitan(mesh, tiny_indices_buffer);

// 				short* indices_data = (short*)tiny_indices_buffer.data.data();
// 				float* position_data = (float*)tiny_positions_buffer.data.data();
// 				float* texcoord_data = (float*)tiny_tex_coords_buffer.data.data();

// 				float* tangent_data = (float*)std::malloc(tiny_indices_acc.count * 3 * sizeof(float));
// 				float* bitangent_data = (float*)std::malloc(tiny_indices_acc.count * 3 * sizeof(float));
				
// 				for(uint i = 0; i < tiny_indices_acc.count; i += 3)
// 				{
					
// 					short i1 = indices_data[i];
// 					short i2 = indices_data[i + 1];
// 					short i3 = indices_data[i + 2];
// 					// calc_tan({
// 					// 		position_data[i2] - position_data[i1],
// 					// 		position_data[i2 + 1] - position_data[i1 + 1],
// 					// 		position_data[i2 + 2] - position_data[i1 + 2]

// 					// 	}, {
// 					// 		position_data[i3] - position_data[i1],
// 					// 		position_data[i3 + 1] - position_data[i1 + 1],
// 					// 		position_data[i3 + 2] - position_data[i1 + 2]
// 					// 	}, {
// 					// 		texcoord_data[i2] - texcoord_data[i1],
// 					// 		texcoord_data[i2 + 1] - texcoord_data[i1 + 1],
// 					// 	}, {
// 					// 		texcoord_data[i3] - texcoord_data[i1],
// 					// 		texcoord_data[i3 + 1] - texcoord_data[i1 + 1],
// 					// 	});
// 					glm::vec3 e1 = {
// 						position_data[i2] - position_data[i1],
// 						position_data[i2 + 1] - position_data[i1 + 1],
...
// 					std::memcpy(&tangent_data[3 * i1], glm::value_ptr(tbn[0]), 3 * sizeof(float));
// 					std::memcpy(&tangent_data[3 * i2], glm::value_ptr(tbn[0]), 3 * sizeof(float));
// 					std::memcpy(&tangent_data[3 * i3], glm::value_ptr(tbn[0]), 3 * sizeof(float));
// 					std::memcpy(&bitangent_data[3 * i1], glm::value_ptr(tbn[1]), 3 * sizeof(float));
// 					std::memcpy(&bitangent_data[3 * i2], glm::value_ptr(tbn[1]), 3 * sizeof(float));
// 					std::memcpy(&bitangent_data[3 * i3], glm::value_ptr(tbn[1]), 3 * sizeof(float));
// 				}

// 				// mesh->tangents_buffer = create_vertex_buffer(tangent_data, tiny_indices_acc.count * 3 * sizeof(float), {{"TANGENT", NOOR_FLOAT, 3, 0}});
// 				// mesh->bitangents_buffer = create_vertex_buffer(bitangent_data, tiny_indices_acc.count * 3 * sizeof(float), {{"BITANGENT", NOOR_FLOAT, 3, 0}});
	...
	Ref<Model> load_model(const char* file_path)
	{
		...
		for (uint32_t i = 0; i < tiny_scene.nodes.size(); i++)
		{
			...
			for (uint32_t i = 0; i < tiny_mesh.primitives.size(); i++)
			{
				...
				{
					tinygltf::Image &tiny_base_image = tiny_model.images[tiny_model.textures[tiny_material.pbrMetallicRoughness.baseColorTexture.index].source];
					Util::Image base_image = Util::flip_image_vertically({(uint32_t)tiny_base_image.width, (uint32_t)tiny_base_image.height, NOOR_UBYTE, (uint32_t)tiny_base_image.component, tiny_base_image.image.data()});
					mesh->material->base_tex = create_texture({0, 0, NOOR_SRGB8, 0, NOOR_LINEAR, NOOR_LINEAR, NOOR_REPEAT}, base_image);
					Util::free_image(base_image);
				}
				...
				{
					tinygltf::Image &tiny_normal_image = tiny_model.images[tiny_model.textures[tiny_material.normalTexture.index].source];
					Util::Image normal_image = Util::flip_image_vertically({(uint32_t)tiny_normal_image.width, (uint32_t)tiny_normal_image.height, NOOR_UBYTE, (uint32_t)tiny_normal_image.component, tiny_normal_image.image.data()});
					mesh->material->normal_tex = create_texture({0, 0, NOOR_RGB8, 0, NOOR_LINEAR, NOOR_LINEAR, NOOR_REPEAT}, normal_image); 
					Util::free_image(normal_image);
				}
				...
				{
					tinygltf::Image &tiny_metallic_roughness_image = tiny_model.images[tiny_model.textures[tiny_material.pbrMetallicRoughness.metallicRoughnessTexture.index].source];
					Util::Image metallic_roughness_image = Util::flip_image_vertically({(uint32_t)tiny_metallic_roughness_image.width, (uint32_t)tiny_metallic_roughness_image.height, NOOR_UBYTE, (uint32_t)tiny_metallic_roughness_image.component, tiny_metallic_roughness_image.image.data()});
					mesh->material->metallic_roughness_tex = create_texture({0, 0, NOOR_RGB8, 0, NOOR_LINEAR, NOOR_LINEAR, NOOR_REPEAT},  metallic_roughness_image);
					Util::free_image(metallic_roughness_image);
				}
